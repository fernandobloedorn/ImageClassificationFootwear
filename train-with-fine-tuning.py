...
def train_recurrent(label, model,cbks):
    ...
    weights_path = get_file('vgg16_weights_tf_dim_ordering_tf_kernels.h5', WEIGHTS_PATH, cache_subdir='./weights')
    ...
    try:
        ...
        pd.DataFrame.from_dict(history.history).to_csv("./history/"+label+"_"+str(epochs)+"_epochs_"+TODAY+'_com_tf_ft_25.csv',index=False)
        ...
        plt.show()
        plt.savefig("./plots/"+label+"_"+str(epochs)+"_epochs_"+TODAY+"_accuracy_com_tf_ft_25.png", bbox_inches='tight')
        plt.clf()
        ...
        plt.legend(['Train master', 'Val master', 'Train sub', 'Val sub'], loc='upper left')
        plt.show()
        plt.savefig("./plots/"+label+"_"+str(epochs)+"_epochs_"+TODAY+"_loss_com_tf_ft_25.png", bbox_inches='tight')
        plt.clf()
    except ValueError as v:
        print(v)

    # Saving the weights in the current directory
    model.save_weights("./weights/"+label+"_"+str(epochs)+"_epochs_"+TODAY+"_com_tf_ft_25.h5")                                        

from cnnfinetuning import Train
